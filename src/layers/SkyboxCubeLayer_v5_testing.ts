...
export class SkyboxCubeLayer {
    ...
    // Individual Panel Texture Methods
    public async setPanelTexture(panelIndex: number, textureUrl: string | null, fitMode: string = 'cover'): Promise<void> {
        if (panelIndex < 0 || panelIndex >= this.panels.length) {
        ...
        const material = panel.material as THREE.MeshLambertMaterial;

        if (textureUrl === null) {
            // Clear texture and restore original panel color
            if (this.panelTextures[panelIndex]) {
                this.panelTextures[panelIndex]!.dispose();
                this.panelTextures[panelIndex] = null;
            }
            ...
            console.log(`🖼️ Cleared texture for panel ${panelIndex} and restored original color`);
        } else {
            try {
                // Load new texture
                const loader = new THREE.TextureLoader();
                ...
                // Configure texture based on fit mode
                this.configureTextureFitMode(texture, fitMode);

                // Apply texture and set panel to white when image is present
                this.panelTextures[panelIndex] = texture;
                material.map = texture;
                material.transparent = true;
                material.opacity = 0.8;

                // Set panel color to white when image is added
                material.color.setHex(0xffffff);
                material.needsUpdate = true;

                console.log(`🖼️ Applied texture to panel ${panelIndex} with fit mode: ${fitMode}, panel color set to white`);
            } catch (error) {
                console.error(`Error loading texture for panel ${panelIndex}:`, error);
            }
        }
    }

    public clearAllPanelTextures(): void {
        this.panels.forEach((panel, index) => {
            const material = panel.material as THREE.MeshLambertMaterial;

            // Dispose texture
            if (this.panelTextures[index]) {
                this.panelTextures[index]!.dispose();
                this.panelTextures[index] = null;
            }

            // Clear texture and restore original panel color
            material.map = null;
            material.color.setHex(this.panelColors[index]);
            material.opacity = 0.7;
            material.needsUpdate = true;
        });

        console.log('🖼️ All panel textures cleared and original colors restored');
    }

    private configureTextureFitMode(texture: THREE.Texture, fitMode: string): void {
        texture.flipY = false;

        switch (fitMode) {
            case 'cover':
                // Fill the entire panel, may crop image
            ...
            case 'contain':
                // Fit entire image within panel, may show panel color around edges
                texture.wrapS = THREE.ClampToEdgeWrapping;
                texture.wrapT = THREE.ClampToEdgeWrapping;
                texture.repeat.set(1, 1);
                texture.offset.set(0, 0);
            ...
            case 'stretch':
                // Stretch image to fill panel exactly (may distort)
                texture.wrapS = THREE.ClampToEdgeWrapping;
                texture.wrapT = THREE.ClampToEdgeWrapping;
                texture.repeat.set(1, 1);
                texture.offset.set(0, 0);
        ...
        console.log(`🖼️ Texture configured for fit mode: ${fitMode}`);
    }

    public resetToOriginalShape(): void {
        // Reset morphing to 0
        this.morphPanelSquareToCircle(0);

        // Reset positions and rotations
        this.panels.forEach((panel, index) => {
            if (this.originalPositions[index] && this.originalRotations[index]) {
                panel.position.copy(this.originalPositions[index]);
                panel.rotation.copy(this.originalRotations[index]);
            }
        });

        // Reset cellular morph state
        this.cellularMorphState.currentLevel = 0;
        this.cellularMorphState.morphProgress = 0;
        this.cellularMorphState.isAnimating = false;
    ...
    public resetRotation(): void {
        this.group.rotation.set(0, 0, 0);
        console.log('🔄 Group rotation reset');
    }
