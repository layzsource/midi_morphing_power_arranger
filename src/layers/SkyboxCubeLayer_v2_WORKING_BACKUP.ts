...
export class SkyboxCubeLayer {
    ...
    constructor(scene: THREE.Scene) {
        ...
        // Set default visible state - JUST A DAMN CUBE
        this.createSimpleCube();
    ...
    private initPanelSystem() {
        ...
        panelConfigs.forEach((config, index) => {
            // Use subdivision geometry for level 0 (base cube) - KEEP THIS WORKING
    ...
    public setMorphProgress(progress: number): void {
        ...
        // Always keep the simple cube visible and just morph IT
        this.createSimpleCube();

        // Morph the solid cube directly
        if (this.solidCube && progress > 0) {
            this.morphSolidCubeToSphere(progress);
        }
    ...
    private createSimpleCube(): void {
        ...
        if (!this.solidCube) {
            ...
            const materials = [
                new THREE.MeshLambertMaterial({ color: 0xffff00, transparent: true, opacity: 0.7 }), // EAST - Yellow
                new THREE.MeshLambertMaterial({ color: 0xff00ff, transparent: true, opacity: 0.7 }), // WEST - Magenta
                new THREE.MeshLambertMaterial({ color: 0x0000ff, transparent: true, opacity: 0.7 }), // TOP - Blue
                new THREE.MeshLambertMaterial({ color: 0xffffff, transparent: true, opacity: 0.7 }), // BOTTOM - White
                new THREE.MeshLambertMaterial({ color: 0xff0000, transparent: true, opacity: 0.7 }), // FRONT - Red
                new THREE.MeshLambertMaterial({ color: 0x00ff00, transparent: true, opacity: 0.7 })  // BACK - Green
            ];
