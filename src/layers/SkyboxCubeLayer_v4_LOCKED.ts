...
export class SkyboxCubeLayer {
    ...
    public async setPanelTexture(panelIndex: number, textureUrl: string | null): Promise<void> {
        if (panelIndex < 0 || panelIndex >= this.panels.length) {
        ...
        if (textureUrl === null) {
            ...
            material.map = null;
            material.needsUpdate = true;
            console.log(`🖼️ Cleared texture for panel ${panelIndex}`);
        } else {
            try {
                // Load new texture
                const loader = new THREE.TextureLoader();
                ...
                console.log(`🖼️ Applied texture to panel ${panelIndex}`);
            } catch (error) {
    ...
    public clearAllPanelTextures(): void {
        ...
        console.log('🖼️ All panel textures cleared');
    }
