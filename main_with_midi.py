...
class MidiMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.note_lights = {}  # Store lights created by MIDI notes
    ...
    def _on_midi_note_on(self, note, velocity):
        """Handle MIDI note on - create a light."""
        try:
    ...
    def _create_note_light(self, note_number, velocity):
        """Create a light for a MIDI note."""
        try:
            ...
            z_position = np.interp(note_number, [0, 127], [self.z_min - 1.0, self.z_max + 1.0])
            
            # Random x,y position but consistent for same note
            np.random.seed(note_number)
            x_position = np.random.uniform(-2.0, 2.0)
            y_position = np.random.uniform(-2.0, 2.0)
            
            # Create light
            light = pv.Light(
                position=(x_position, y_position, z_position),
                color=rgb_color,
                intensity=velocity * 10.0,  # Scale intensity
                positional=True
            )
    ...
    def _remove_light(self, note_number):
        ...
        if note_number in self.note_lights:
            ...
            self.plotter_widget.remove_light(light_info['light'])
