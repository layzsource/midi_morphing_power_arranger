#!/usr/bin/env python3
"""
Enhanced MIDI Morphing Visualizer - Step 5: Advanced Lighting and Visual Effects (COMPLETE FIXED)
...
🎬 Enhanced Animation System with Keyframe Interpolation
...
- Added missing imports (QActionGroup)
- Enhanced error handling for audio/MIDI initialization
- Improved 3D compatibility with PyVista
- Fixed geometry library subdivision issues
...
class AdvancedLightingSystem:
    ...
    def add_light(self, light_type: LightType, position: np.ndarray, 
        ...
        try:
            if self.plotter and hasattr(self.plotter, 'add_light'):
    ...
    def remove_light(self, light_id: str):
        ...
        if light_id in self.lights:
            ...
            try:
                if light.pv_light and self.plotter and hasattr(self.plotter, 'remove_light'):
    ...
    def render_particles(self):
        ...
        try:
            ...
            for actor in self.particle_actors:
                try:
                    if self.plotter and hasattr(self.plotter, 'remove_actor'):
                        self.plotter.remove_actor(actor)
                except:
                    pass
            self.particle_actors.clear()
            
            if not self.particles or not self.plotter:
                return
...
class VisualEffectsManager:
    ...
    def _apply_fog(self):
        ...
        if fog_effect.enabled and self.plotter:
            ...
            current_bg = getattr(self.plotter, 'background_color', [0.2, 0.2, 0.2])
...
class GeometryLibrary:
    ...
    def _create_base_shapes(self):
        ...
        try:
            ...
            # Create cube - using simpler approach to avoid subdivision errors
            cube = pv.Box(bounds=[-1, 1, -1, 1, -1, 1])
            try:
                cube_surface = cube.extract_surface()
                cube_triangulated = cube_surface.triangulate()
                self.base_meshes['cube'] = cube_triangulated
            except:
                # Fallback to basic cube
                self.base_meshes['cube'] = cube
            ...
            try:
                ...
                # Fallback torus using different method
                torus = pv.Sphere(radius=1.0, theta_resolution=12, phi_resolution=8)
                self.base_meshes['torus'] = torus
    ...
    def _create_fallback_shapes(self):
        ...
        # Simple cone
        cone = pv.Cone(radius=1.0, height=2.0, resolution=10)
        self.base_meshes['cone'] = cone
        
        # Simple cylinder
        cylinder = pv.Cylinder(radius=1.0, height=2.0, resolution=10)
        self.base_meshes['cylinder'] = cylinder
        
        # Simple torus fallback (use sphere)
        self.base_meshes['torus'] = sphere.copy()
        
        print("⚠️ Using fallback geometry")
    
    def morph_between_shapes(self, shape1_name, shape2_name, alpha):
        """Morph between two shapes with improved compatibility"""
        ...
        try:
            ...
            if source_mesh.n_points != target_mesh.n_points:
                # Try to resample to match point counts
                if source_mesh.n_points < target_mesh.n_points:
                    # Use source mesh structure
                    return source_mesh.copy()
                else:
                    # Use target mesh structure
...
class EnhancedMorphingSceneObject:
    ...
    def __init__(self, name, position, note_range, color, geometry_lib):
        ...
        # Animation properties - FIXED: Initialize with proper dtypes
        self.scale_animation = 1.0
        self.rotation = np.array([0.0, 0.0, 0.0], dtype=np.float64)
        self.rotation_speed = np.array([0.0, 0.5, 0.0], dtype=np.float64)
...
class EnhancedAudioAnalyzer(QObject):
    ...
    def _sounddevice_loop(self):
        """SoundDevice backend loop with error handling"""
        try:
            import sounddevice as sd
    ...
    def _sounddevice_callback(self, indata, frames, time, status):
        ...
        try:
            ...
            print(f"SoundDevice callback error: {e}")
    
    def _pyaudio_loop(self):
        """PyAudio backend loop with error handling"""
        try:
            import pyaudio
...
class MIDIHandler(QObject):
    ...
    def _scan_devices(self):
        """Scan for available MIDI devices with error handling"""
        if not MIDI_AVAILABLE:
            print("⚠️ MIDI not available")
            return
        ...
        try:
            ...
            print(f"✅ Found {len(self.midi_devices)} MIDI input devices")
    ...
    def _process_midi_event(self, event):
        ...
        try:
            ...
            if message_type == 0x90:  # Note on
                ...
                controller = note  # Second byte is controller number
                value = velocity   # Third byte is value
    ...
    def test_note(self, note):
        """Test a note without MIDI input"""
        velocity = 0.8
        self.note_on.emit(note, velocity, 0)
...
class QtInteractorWrapper:
    """Fixed wrapper for QtInteractor to handle compatibility issues"""
    ...
    def _create_plotter(self):
        """Create QtInteractor with proper compatibility handling"""
        try:
            # Create QtInteractor - it IS the plotter in newer versions
            self.widget = QtInteractor(self.parent)
            
            # FIXED: QtInteractor IS the plotter, not widget.plotter
            ...
            if self.plotter:
                try:
                    ...
                    print("⚠️ Could not set background")
                ...
                try:
                    ...
                    print("⚠️ Could not enable anti-aliasing")
...
class Step5MorphingMainWindow(QMainWindow):
    """Enhanced main window for Step 5 with advanced lighting and effects"""
    
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Enhanced MIDI Morphing Visualizer - Step 5: Advanced Lighting & Effects")
        ...
        self._create_enhanced_systems()
        self._connect_enhanced_signals()
        self._setup_enhanced_shortcuts()
        
        print("✅ Enhanced MIDI Morphing Visualizer - Step 5 initialized")
    
    def _create_enhanced_ui(self):
        """Create enhanced UI with advanced controls"""
        try:
    ...
    def _create_control_panel(self):
        ...
        lighting_group = QGroupBox("🎆 Advanced Lighting")
        ...
        self.ambient_label = QLabel("Ambient: 30%")
        lighting_layout.addWidget(self.ambient_label)
        lighting_layout.addWidget(self.ambient_slider)
        
        # Dynamic lighting checkbox
        self.dynamic_lighting_cb = QCheckBox("Dynamic Lighting Effects")
        ...
        # Particles enabled
        self.particles_enabled_cb = QCheckBox("Particles Enabled")
        self.particles_enabled_cb.setChecked(True)
        particle_layout.addWidget(self.particles_enabled_cb)
        
        # Particle emission rate
        self.particle_rate_slider = QSlider(Qt.Horizontal)
        self.particle_rate_slider.setRange(0, 200)
        self.particle_rate_slider.setValue(50)
        self.particle_rate_label = QLabel("Emission Rate: 50")
        particle_layout.addWidget(self.particle_rate_label)
        particle_layout.addWidget(self.particle_rate_slider)
        ...
        self.bloom_label = QLabel("Bloom Intensity: 30%")
        effects_layout.addWidget(self.bloom_label)
        effects_layout.addWidget(self.bloom_slider)
        
        layout.addWidget(effects_group)
        
        # Audio Reactive Controls
        audio_group = QGroupBox("🎵 Audio Reactive")
        ...
        self.audio_sensitivity_label = QLabel("Audio Sensitivity: 100%")
        audio_layout.addWidget(self.audio_sensitivity_label)
        ...
        test_buttons = [
            ("Test Morphing Sequence", self._test_morphing),
            ("Test Lighting Effects", self._test_lighting),
            ("Test Particle Burst", self._test_particles),
            ("Test Audio Features", self._test_audio),
            ("Test MIDI Notes", self._test_midi),
            ("Visual Burst Effect", self._trigger_visual_burst)
        ]
        ...
        for text, func in preset_buttons:
            btn = QPushButton(text)
            btn.clicked.connect(func)
    ...
    def _create_3d_panel(self):
        ...
        try:
            ...
            fallback = QWidget()
            fallback.setStyleSheet("background-color: #2a2a2a; color: white;")
            layout = QVBoxLayout(fallback)
            label = QLabel("3D Visualization Error")
            label.setAlignment(Qt.AlignCenter)
            layout.addWidget(label)
    ...
    def _create_enhanced_menu_bar(self):
        ...
        for quality in ["low", "medium", "high", "ultra"]:
            ...
            action.setChecked(quality == "high")
    ...
    def _create_enhanced_status_bar(self):
        """Create enhanced status bar with performance info"""
    ...
    def _create_enhanced_systems(self):
        """Create enhanced backend systems"""
        try:
            # Performance monitor
            ...
            print("✅ Enhanced systems created")
    ...
    def _connect_enhanced_signals(self):
        ...
        try:
            ...
            # Lighting controls
            self.lighting_combo.currentTextChanged.connect(self._on_lighting_preset_changed)
            ...
            print("✅ Enhanced signals connected")
    ...
    def _setup_enhanced_shortcuts(self):
        ...
        try:
            ...
            burst_shortcut.activated.connect(self._trigger_visual_burst)
            
            print("✅ Enhanced shortcuts setup")
    ...
    def _on_global_morph_changed(self, value):
        ...
        factor = value / 100.0
        self.global_morph_factor = factor
    ...
    def _on_ambient_changed(self, value):
        ...
        if self.scene_manager and hasattr(self.scene_manager, 'lighting_system'):
            self.scene_manager.lighting_system.ambient_level = value / 100.0
    
    def _on_particle_rate_changed(self, value):
        """Handle particle emission rate change"""
        self.particle_rate_label.setText(f"Emission Rate: {value}")
    ...
    def _on_bloom_changed(self, value):
        ...
        self.bloom_label.setText(f"Bloom Intensity: {value}%")
