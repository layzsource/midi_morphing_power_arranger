...
class MidiMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.note_spheres = {}  # Store spheres created by MIDI notes (instead of lights)
        ...
        # Timer for sphere cleanup
        self.cleanup_timer = QTimer()
        self.cleanup_timer.timeout.connect(self._cleanup_expired_spheres)
    ...
    def _setup_ui(self):
        ...
        # Sphere count display
        self.sphere_count_label = QLabel("Active Note Spheres: 0")
        self.layout.addWidget(self.sphere_count_label)
        
        # Clear spheres button
        self.clear_spheres_button = QPushButton("Clear All Note Spheres")
        self.clear_spheres_button.clicked.connect(self._clear_all_spheres)
        self.layout.addWidget(self.clear_spheres_button)

    def _initialize_visualization(self):
        """Initialize the 3D visualization."""
        self.current_mesh = self.initial_meshes[self.current_mesh_key].copy()
        
        # Get bounds for sphere positioning
        ...
        self.status_bar.showMessage("Visualization ready")
    ...
    def _setup_midi(self):
        ...
        if self.midi_handler.start(self.config.MIDI_PORT):
            self.status_bar.showMessage("MIDI connected successfully")
        else:
    ...
    def _on_midi_note_on(self, note, velocity):
        """Handle MIDI note on - create a colored sphere."""
        try:
            # Remove existing sphere for this note
            if note in self.note_spheres:
                self._remove_sphere(note)
            
            # Create new sphere
            sphere_actor = self._create_note_sphere(note, velocity)
            if sphere_actor:
                self.note_spheres[note] = {
                    'actor': sphere_actor,
                    'timestamp': time.time(),
                    'velocity': velocity
                }
                
                self._update_sphere_count()
    ...
    def _on_midi_note_off(self, note):
        """Handle MIDI note off - remove sphere."""
        try:
            if note in self.note_spheres:
                self._remove_sphere(note)
                self._update_sphere_count()
    ...
    def _create_note_sphere(self, note_number, velocity):
        """Create a colored sphere for a MIDI note."""
        try:
            # Map note to hue (0-127 notes to 0-1 hue)
            hue = note_number / 127.0
            rgb_color = colorsys.hsv_to_rgb(hue, 1.0, 1.0)
            
            # Position based on note number
            z_position = np.interp(note_number, [0, 127], [self.z_min - 2.0, self.z_max + 2.0])
            
            # Random x,y position but consistent for same note
            np.random.seed(note_number)
            x_position = np.random.uniform(-3.0, 3.0)
            y_position = np.random.uniform(-3.0, 3.0)
            
            # Create sphere with size based on velocity
            radius = 0.1 + (velocity * 0.3)  # 0.1 to 0.4 radius
            sphere = pv.Sphere(radius=radius, center=(x_position, y_position, z_position))
    ...
    def _remove_sphere(self, note_number):
        """Remove a sphere by note number."""
        if note_number in self.note_spheres:
            sphere_info = self.note_spheres.pop(note_number)
            try:
                self.plotter_widget.remove_actor(sphere_info['actor'])
            except Exception as e:
                print(f"Error removing sphere actor: {e}")
    
    def _cleanup_expired_spheres(self):
        """Remove spheres that have been on too long."""
        current_time = time.time()
        expired_notes = []
        
        for note, sphere_info in self.note_spheres.items():
            if current_time - sphere_info['timestamp'] > 30:  # 30 second timeout
    ...
    def _clear_all_spheres(self):
        """Remove all spheres."""
        for note in list(self.note_spheres.keys()):
            self._remove_sphere(note)
        self._update_sphere_count()
        self.plotter_widget.render()
    
    def _update_sphere_count(self):
        """Update the sphere count display."""
        count = len(self.note_spheres)
        self.sphere_count_label.setText(f"Active Note Spheres: {count}")
