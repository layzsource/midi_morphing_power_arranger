#!/usr/bin/env python3
"""
Standalone MIDI Morphing Visualizer - Fixed Morphing Version
Run this file directly: python standalone_app_fixed.py
"""
...
class SimpleConfig:
    def __init__(self):
        self.MESH_RESOLUTION = 30  # Lower resolution for better compatibility
        self.MIDI_PORT = None
...
# FIXED geometry functions for smooth morphing
def create_matched_meshes(resolution=30):
    """Create meshes with matched vertex counts for smooth morphing."""
    try:
        meshes = {}
        ...
        print(f"Target vertex count for all meshes: {target_points}")
        
        # Create and match cube
        cube = pv.Cube()
        cube_detailed = cube.subdivide(3)
        if cube_detailed.n_points > target_points:
            ratio = target_points / cube_detailed.n_points
            meshes['cube'] = cube_detailed.decimate(1 - ratio)
        else:
            # If still too few points, subdivide more
            while cube_detailed.n_points < target_points * 0.8:
                cube_detailed = cube_detailed.subdivide(1)
            meshes['cube'] = cube_detailed
        
        # Create and match cone
        cone = pv.Cone(resolution=resolution)
        cone_detailed = cone.subdivide(2)
        if cone_detailed.n_points > target_points:
            ratio = target_points / cone_detailed.n_points
            meshes['cone'] = cone_detailed.decimate(1 - ratio)
        else:
            while cone_detailed.n_points < target_points * 0.8:
                cone_detailed = cone_detailed.subdivide(1)
            meshes['cone'] = cone_detailed
        
        # Create and match cylinder
        try:
            cylinder = pv.Cylinder(resolution=resolution)
            cylinder_detailed = cylinder.subdivide(2)
            if cylinder_detailed.n_points > target_points:
                ratio = target_points / cylinder_detailed.n_points
                meshes['cylinder'] = cylinder_detailed.decimate(1 - ratio)
            else:
                while cylinder_detailed.n_points < target_points * 0.8:
                    cylinder_detailed = cylinder_detailed.subdivide(1)
                meshes['cylinder'] = cylinder_detailed
        except:
            print("Cylinder creation failed, skipping")
        
        # Create manual torus with matching vertex count
        try:
            torus = create_manual_torus_matched(target_points)
            meshes['torus'] = torus
        except:
            print("Torus creation failed, skipping")
        
        # Print final vertex counts to verify matching
        print("Final mesh vertex counts:")
        ...
        if len(set(vertex_counts)) == 1:
            print("✓ All meshes have identical vertex counts - smooth morphing enabled!")
        else:
            print("⚠ Some vertex counts differ - morphing may be choppy")
...
def create_manual_torus_matched(target_points):
    """Create a torus with approximately target_points vertices."""
    try:
        # Calculate grid size to get approximately target_points
        grid_size = int(np.sqrt(target_points / 2))
        ...
        # Create structured grid for better surface
        mesh = pv.StructuredGrid()
        mesh.points = points
        mesh.dimensions = [grid_size, grid_size, 1]
        
        # Convert to unstructured for easier manipulation
        torus = mesh.cast_to_unstructured_grid()
        
        return torus
        
    except Exception as e:
        print(f"Manual torus creation failed: {e}")
        # Return scaled cylinder as fallback
        return pv.Cylinder(radius=0.3, height=0.1).scale([3, 3, 1])

def blend_meshes_smooth(meshes, source_key, target_key, alpha):
    """Smooth mesh blending for meshes with identical vertex counts."""
    ...
    if source_mesh.n_points != target_mesh.n_points:
        print(f"Warning: Vertex count mismatch: {source_mesh.n_points} vs {target_mesh.n_points}")
        print("Using source mesh only - consider using create_matched_meshes()")
    ...
    # Optional: Add some smoothing to prevent artifacts
    return blended_points

# Simple Audio Analyzer (unchanged)
...
class MorphingVisualizerWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("MIDI Morphing Visualizer - Smooth Morphing Version")
        ...
        print("Initializing 3D visualization with matched meshes...")
        ...
        print("✓ Application initialized successfully with smooth morphing!")
    
    def _initialize_visualization(self):
        try:
            print("Creating matched meshes for smooth morphing...")
            self.initial_meshes = create_matched_meshes(self.config.MESH_RESOLUTION)
            print(f"✓ Created {len(self.initial_meshes)} matched meshes")
    ...
    def _setup_ui(self):
        ...
        self.status_bar.showMessage("Ready - Smooth morphing enabled!")
    ...
    def _on_morph_change(self, value):
        alpha = value / 100.0
        try:
            blended_points = blend_meshes_smooth(
                self.initial_meshes, 
            ...
            if blended_points is not None:
                ...
                # Show morph progress in status bar
...
def main():
    print("=== MIDI Morphing Visualizer - Smooth Morphing Version ===")
    ...
    try:
        ...
        print("\nUsage:")
        print("- Use the morph slider to smoothly blend between shapes")
        ...
        print("- Smooth morphing is now enabled between all shapes!")
